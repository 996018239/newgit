...
animate:function (obj,moveJson,speed,fn){//对象 运动模式 移动时间 回调
	...
	//如果输入预定速度的字符串，就进行转换
	if(speed){
		if(typeof speed=='string') speed=prd_speed[speed];
	}else{
...
function LBT(obj){
		...
		//var oDiv=hxsd_tools.getByClass(document,'box')[0];
		var op=obj.getElementsByTagName('p')[0];
		var img=op.children;
		var ol=obj.getElementsByTagName('ol')[0];
		var aBtn=obj.getElementsByClassName('btn');
		var n=0;
		var Li_w=hxsd_tools.getStyle(img[0],'width');
		op.style.width=Li_w*img.length+'px';
		...
		for(var i=0;i<oBtn.length;i++){
			...
			oBtn[i].onclick=function(){
				n=this.index;
				ac(n);
				hxsd_tools.animate(op,{'left':-this.index*Li_w});
			};
		};
		//-----------------------------------
		function ac(n){
		...
		aBtn[0].onclick=function(){
			...
			if(n<0) n=0;
			ac(n);
			hxsd_tools.animate(op,{'left':-n*Li_w});
		};
		var m=img.length/2
